...
using YoutubeDownloader.Converter;

using YouTubeDownloader.Videos.Streams;

namespace YouTubeDownloader.Converter.Tests
{
    public class SubtitleSpecs : IClassFixture<TempOutputFixture>, IClassFixture<FFmpegFixture>
    {
        ...
        public async Task User_can_download_a_video_with_subtitles_into_a_single_mp4_file()
        {
            ...
            streamManifest.GetVideoStreams()
                .OrderBy(s => s.Size)
                .First(s => s.Container == Container.Mp4)
        };
        ...
        public async Task User_can_download_a_video_with_subtitles_into_a_single_webm_file()
        {
            ...
            var streamManifest = await youtube
                .Videos
                .Streams
                .GetManifestAsync("YltHGKX80Y8");

            var trackManifest = await youtube
                .Videos
                .ClosedCaptions
                .GetManifestAsync("YltHGKX80Y8");
            ...
            streamManifest.GetVideoStreams()
                .OrderBy(s => s.Size)
                .First(s => s.Container == Container.WebM)
        };
            ...
            foreach (var trackInfo in trackInfos)
                FileEx.ContainsBytes(outputFilePath,
                    Encoding.ASCII.GetBytes(trackInfo.Language.Name)).Should().BeTrue();
        }
    }
}
