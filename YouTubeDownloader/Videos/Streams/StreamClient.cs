            ...
            CancellationToken cancellationToken = default)
        {
            foreach (var streamInfoExtractor in streamInfoExtractors)
            {
                var itag =
                    ...
                    throw new YoutubeDownloaderException("Could not extract stream itag.");

                var url =
                    streamInfoExtractor.TryGetUrl() ??
                    throw new YoutubeDownloaderException("Could not extract stream URL.");
                ...
                var container =
                    ...
                    throw new YoutubeDownloaderException("Could not extract stream container.");

                var bitrate =
                    streamInfoExtractor.TryGetBitrate()?.Pipe(s => new Bitrate(s)) ??
                    throw new YoutubeDownloaderException("Could not extract stream bitrate.");
