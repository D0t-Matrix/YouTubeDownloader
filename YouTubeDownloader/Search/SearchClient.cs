            ...
            do
            {
                ...
                foreach (var videoExtractor in searchResults.GetVideos())
                {
                    ...
                    var id =
                        ...
                        throw new YoutubeDownloaderException("Could not extract video ID.");
                    ...
                    var title =
                        ...
                        throw new YoutubeDownloaderException("Could not extract video title.");

                    var channelTitle =
                        videoExtractor.TryGetVideoAuthor() ??
                        throw new YoutubeDownloaderException("Could not extract video author.");

                    var channelId =
                        videoExtractor.TryGetVideoChannelId() ??
                        throw new YoutubeDownloaderException("Could not extract video channel ID.");
                    ...
                    foreach (var thumbnailExtractor in videoExtractor.GetVideoThumbnails())
                    {
                        var thumbnailUrl =
                            ...
                            throw new YoutubeDownloaderException("Could not extract thumbnail URL.");

                        var thumbnailWidth =
                            thumbnailExtractor.TryGetWidth() ??
                            throw new YoutubeDownloaderException("Could not extract thumbnail width.");

                        var thumbnailHeight =
                            thumbnailExtractor.TryGetHeight() ??
                            throw new YoutubeDownloaderException("Could not extract thumbnail height.");
                ...
                foreach (var playlistExtractor in searchResults.GetPlaylists())
                {
                    ...
                    var id =
                        ...
                        throw new YoutubeDownloaderException("Could not extract playlist ID.");
                    ...
                    var title =
                        ...
                        throw new YoutubeDownloaderException("Could not extract playlist title.");
                ...
                foreach (var channelExtractor in searchResults.GetChannels())
                {
                    ...
                    var channelId =
                        ...
                        throw new YoutubeDownloaderException("Could not extract channel ID.");

                    var title =
                        channelExtractor.TryGetChannelTitle() ??
                        throw new YoutubeDownloaderException("Could not extract channel title.");
